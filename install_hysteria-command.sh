...
header() {
  ...
  echo -e "${BLUE}▪ 系统: ${YELLOW}$(grep PRETTY_NAME /etc/os-release | cut -d= -f2 | tr -d '"')${NC}"
  separator
}
...
install() {
  ...
  for pkg in wget openssl; do
    if ! command -v "$pkg" &>/dev/null; then
      apk add --no-cache "$pkg" >/dev/null || { error "依赖安装失败"; return 1; }
      success "已安装依赖: $pkg"
    else
      info "依赖已存在: $pkg"
    fi
...
EOF
  ...
  /etc/init.d/hysteria start >/dev/null || { error "服务启动失败"; return 1; }
  success "系统服务已配置"
